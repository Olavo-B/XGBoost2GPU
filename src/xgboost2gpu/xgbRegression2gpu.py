# /*****************************************************************************/
#  * File: xgboostregressor2gpu.py
#  * Author: Adapted from Olavo Alves Barros Silva
#  * Contact: olavo.barros@ufv.com
#  * Date: 2025-09-09
#  * License: [License Type]
#  * Description: This is the main class to generate the cuda code 
#  * for the XGBoost regression algorithm.
# /*****************************************************************************/
...
class XGBoostRegressor2GPU:
    def __init__(self, xgb_regressor: XGBRegressor,
                 w_feature: int = 3,
                 w_tree: int = 3,
                 n_samples: int = 1000,
                 n_threads: int = 256,
        ...
        self._debug             = debug
        self._n_classes         = 1  # For regression, we have a single output

        # Extract trees and parameters from XGBoost regressor
        self._trees             = self._extract_trees_from_xgb()
        self._base_score        = xgb_regressor.get_params().get('base_score', 0.5)
        self._min, self._max    = self._calculate_feature_bounds()
    ...
    @model.setter
    def model(self, value: XGBRegressor):
        if not isinstance(value, XGBRegressor):
            raise TypeError("Expected an XGBRegressor instance.")
        self._model = value
        # Update extracted data when model changes
        self._trees = self._extract_trees_from_xgb()
        self._base_score = value.get_params().get('base_score', 0.5)
        self._min, self._max = self._calculate_feature_bounds()

    #===================#
    # Private Methods   #
    #===================#
    def _extract_trees_from_xgb(self):
        """Extract tree structure from XGBoost regressor."""
        if not hasattr(self._model, 'get_booster'):
            raise ValueError("XGBoost model must be fitted before extracting trees.")
        ...
        for tree_id in trees_df['Tree'].unique():
            ...
            for _, row in tree_data.iterrows():
                ...
                if pd.isna(row['Split']):  # Leaf node
                    tree_dict[node_id] = {
                        'type': 'leaf',
                        'value': row['Gain'],  # For regression, use Gain as the prediction value
                        'global_id': global_node_id
                    }
                else:  # Split node
                    tree_dict[node_id] = {
                        'type': 'split',
                        'feature': int(row['Split'].replace('f', '')),  # Remove 'f' prefix
                        'threshold': row['Split_Condition'],
                        'yes': row['Yes'],
                        'no': row['No'],
                        'value': row['Gain'],  # Split nodes also have values in regression
                        'global_id': global_node_id
                    }
    ...
    def _calculate_feature_bounds(self):
        """Calculate feature bounds from the training data or use reasonable defaults."""
        # Since we don't have access to training data, we'll use reasonable defaults
        # In a real implementation, you should pass the training data bounds
        n_features = self._model.n_features_in_
        min_vals = np.zeros(n_features)
        max_vals = np.ones(n_features) * 100  # Adjust based on your data scale
        return min_vals, max_vals

    def nodes(self):
        """Return the number of nodes in each tree."""
        return [len(tree) for tree in self._trees]
    ...
    def generate_cuda_code(self, output_file: str, optimization: bool = True):
        """Generate the CUDA code for the XGBoost regression model and save it to a file.
        Args:
            output_file (str): The path to the output file where the CUDA code will be saved.
            optimization (bool): Whether to use optimized version with bit-packed pruning.
        """
    ...
    def calculate_forest_probabilities(self, percentage_to_cut, strategy="adaptive",
        ...
        total_nodes_in_forest = sum(self.nodes())
        ...
        probabilities = [0 for i in range(sum(self.nodes()))]
        ...
        for tree_index, tree in enumerate(self._trees):
            ...
            for node_id, node in reversed(tree.items()):
                # If node is a leaf, the probability is 0.0 (no cut)
                if node['type'] == 'leaf':
        ...
        self._prune = np.array(probabilities)
    ...
    def should_cut_nodes(self, thread_id: int) -> np.ndarray:
        """Determine if a set of nodes should be cut (vectorized version)."""
    ...
    def prune_matrix(self, num_threads: int = None, save_matrix: bool = False) -> np.ndarray:
        ...
        if num_threads is None:
            num_threads = self._n_threads * self._n_blocks
    ...
    def predict(self, X: np.ndarray, thread_id: int = 0) -> np.ndarray:
        """Predict the regression values for the input data using the pruned trees."""
        if not hasattr(self, '_prune') or self._prune is None:
            raise ValueError("Prune vector is not set. Please calculate cut probabilities first.")
        if not hasattr(self, '_trees_array') or self._trees_array is None:
        ...
        predictions = np.full(n_samples, self._base_score, dtype=np.float64)
        ...
        for i, root_node_idx in enumerate(self._tree_root_indices):
            ...
            iteration = 0
            while np.any(active_mask) and iteration < 100:
                iteration += 1
                ...
                # Check if nodes should be finished (pruned or leaf)
                ...
                if np.any(is_finished):
                    ...
                    # Add the prediction values (for regression, sum all tree outputs)
                    predictions[finished_sample_indices] += finished_values
                ...
                # Continue navigation for non-finished nodes
    ...
    def _convert_trees_to_numpy(self):
        """Convert the list of dictionary-based trees into flat NumPy arrays."""
...
"""
    ...
    def _kernel_code(self):
        """Generate the kernel code for regression (modified from classification)."""
        code = "__global__ void xgboost_regression_kernel(int* X, float* Y, float* Y_expected, float* prune, int total_threads, int sample_size) {\n"
        ...
        code += f"   float predictions[{len(self._trees)}];\n"
        code += f"   float mse_sum = 0.0f;\n"
        ...
        for tree_id, tree in enumerate(self._trees):
            ...
            for i, node_id in enumerate(non_root_nodes):
                ...
                if i == 0:
                    code += f"   float node_{global_id}"
                elif i == len(non_root_nodes) - 1:
                    code += f", node_{global_id};\n"
                else:
                    code += f", node_{global_id}"
            
            nodes_offset += self.nodes()[tree_id]
        ...
        code += "        }\n"
        code += "        __syncthreads();\n"
        code += f"        float prediction = {self._base_score}f;\n"

        # Generate forest code
        code += f"      {self._forest_code()}"
        
        # Sum all tree predictions
        for tree_id in range(len(self._trees)):
            code += f"        prediction += predictions[{tree_id}];\n"
        
        # Calculate MSE
        code += "        float error = prediction - Y_expected[i];\n"
        code += "        mse_sum += error * error;\n"
        code += "    }\n"
        code += "    Y[idx] = mse_sum / sample_size;\n"  # Store MSE for this thread
        code += "}\n\n"
        
        return code

    def _main_code(self):
        """Generate main function for regression."""
        code = "int main() {\n"
        ...
        code += f"    const int sample_size = {self._n_samples};\n"
        code += f"    const int prune_size = {sum(self.nodes())};\n\n"

        code += "    int* X;\n"
        code += "    float* Y;\n"  # Changed to float for MSE values
        code += "    float* Y_expected;\n"  # Changed to float for regression targets
        code += "    float* prune;\n\n"

        code += f"    int* X_host = (int*)malloc(sample_size * sizeof(int) * {len(self._min)});\n"
        code += "    float* Y_host = (float*)malloc(sizeof(float) * totalThreads);\n"  # Changed to float
        code += "    float* Y_expected_host = (float*)malloc(sample_size * sizeof(float));\n"  # Changed to float
        code += "    float* prune_host = (float*)malloc(prune_size * sizeof(float));\n\n"

        code += f"    read_vector_from_csv<int>(X_host, \"input.csv\", sample_size, {len(self._min)});\n"
        code += "    read_vector_from_csv<float>(Y_expected_host, \"expected_output.csv\", sample_size, 1);\n"
        code += "    read_vector_from_csv<float>(prune_host, \"prune.csv\", prune_size, 1);\n\n"

        # CUDA memory allocation
        code += f"    cudaMalloc((void**)&X, sample_size * sizeof(int) * {len(self._min)});\n"
        code += "    cudaMalloc((void**)&Y, totalThreads * sizeof(float));\n"
        code += "    cudaMalloc((void**)&Y_expected, sample_size * sizeof(float));\n"
        ...
        code += "    cudaMemcpy(Y_expected, Y_expected_host, sample_size * sizeof(float), cudaMemcpyHostToDevice);\n"
        code += "    cudaMemcpy(prune, prune_host, prune_size * sizeof(float), cudaMemcpyHostToDevice);\n\n"

        code += "    cudaMemset(Y, 0, totalThreads * sizeof(float));\n\n"
        ...
        code += "    xgboost_regression_kernel<<<optimalBlocks, threadsPerBlock>>>(X, Y, Y_expected, prune, totalThreads, sample_size);\n"
        ...
        # Copy results back
        code += "    cudaMemcpy(Y_host, Y, totalThreads * sizeof(float), cudaMemcpyDeviceToHost);\n\n"

        # Find best MSE results
        code += "    std::vector<std::pair<float, int>> thread_mse(totalThreads);\n"
        code += "    for (int i = 0; i < totalThreads; i++) {\n"
        code += "        thread_mse[i] = std::make_pair(Y_host[i], i);\n"
        code += "    }\n"
        code += "    std::sort(thread_mse.begin(), thread_mse.end());\n"  # Sort ascending for MSE

        # Save results
        code += "    FILE *file = fopen(\"mse_results.txt\", \"w\");\n"
        code += "    if (file == NULL) {\n"
        code += "        printf(\"Error opening file for writing MSE results\\n\");\n"
        code += "        exit(EXIT_FAILURE);\n"
        code += "    }\n"

        code += "    fprintf(file, \"id mse\\n\");\n"
        code += "    for (int i = 0; i < totalThreads; i++) {\n"
        code += "        float mse = thread_mse[i].first;\n"
        code += "        int thread_id = thread_mse[i].second;\n"
        code += "        fprintf(file, \"%d %.6f\\n\", thread_id, mse);\n"
        code += "    }\n"
        code += "    fclose(file);\n\n"
